...
constant SOURCE_CHUNK = 10000  -- size of one chunk of memory, in bytes
...
-- begin mwl
global procedure rollback_source( integer line )
	integer chunk, start
...
-- add source line to the list 
    ...
    options = 0
    -- begin mwl
    -- keep the line if preprocessing   
...
-- open an include file (new_include_name) according to the include path rules  
    ...
    if try != -1 then
		...
		-- begin mwl
		if full_path[1] = '\\' then
			-- DOS/Windows, and using the root directory.  Need to
...
global function equal_ins(sequence a, sequence b)
-- return TRUE if two strings are equal, ignoring upper/lower case
...
-- (if any)  
    sequence top
-- begin mwl
-- don't call this, or eval can't see local symbols
-- this is probably on there for speed...
--    HideLocals()
-- end mwl
	-- begin mwl
